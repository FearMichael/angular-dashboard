...
import { IDashboardTableData } from 'src/app/interfaces/dashboard.interfaces';
...
export class DashboardComponent implements OnInit {
  ...
  public filterTable(data: IDashboardTableData[]) {
    ...
    return filterKeys.length ? this._tableData.filter((data: Record<string, any>) => {
      const shouldReturn = filterKeys.some((k) => {
  ...
  public dateFilterTable(data: IDashboardTableData[]) {
    ...
    const filterKeys = Object.keys(this.tableDateFilter).filter((k) => !!this.tableDateFilter[k].startDate || !!this.tableDateFilter[k].endDate);
    const filtered = filterKeys.length ? this._tableData.filter((record) => {
      return filterKeys.some((k) => {
        const val = (record as any)[k];
  ...
  public summaryData = [];
  public summaryHeaders: ITableHeader[] = [
    { value: "title", text: 'Title' },
  ]
